...
function event_timer(e)
	if e.timer == trap_timer then
		...
		for npc in npc_list.entries do
			...
			if npc.valid and npc:GetID() ~= e.self:GetID() and npc:GetID() ~= owner:GetID() and is_client_or_clients_pet == false and e.self:CheckLoS(npc) and npc:GetHP() > 0 then
		...
		if best_npc_id ~= -1 and owner.valid then
			e.self:CastSpell(spell_id, best_npc_id)
