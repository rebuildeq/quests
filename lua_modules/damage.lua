...
function damage.OnDeathTouch(e)
	local dt_spells = {
        982, -- cazic touch
        1948, -- Destroy
...
---@param e ModCommonDamage
function damage.OnPetIncomingDamage(e)
	local attacker = e.attacker
	local defender = e.self

    if not defender:IsNPC() then
        return e
    end
    if not defender:HasOwner() then
        return e
    end

    if not defender:GetOwner():IsClient() then
        return e
    end
    ...
    e.return_value = e.value + e.value * 0.3
    -- eq.debug("pet got dealt damage: " .. e.value .. " new value: " .. e.return_value)
    e.ignore_default = true
end
